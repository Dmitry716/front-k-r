...
const LandscapeCategoryPage = () => {
  ...
  const [products, setProducts] = useState<LandscapeItem[]>([]);
    ...
    const loadProducts = async () => {
      try {
        ...
        if (data.success && data.data) {
          ...
          const actualCategory = CATEGORY_MAP[categorySlug?.toLowerCase()];
          const filteredProducts = data.data.filter(
            (product: LandscapeItem) => product.category === actualCategory
          );
          setProducts(filteredProducts);
        }
      } catch (error) {
        console.error("Ошибка при загрузке товаров landscape:", error);
      } finally {
        setLoading(false);
      }
    };
      ...
      <section className="page-centered mt-5 max-w-[1300px] flex">
        ...
        <div className="w-full lg:ml-5 lg:max-w-[75%]">
          <div className={`${isTablet ? 'container-centered' : ''}`}>
            ...
            {/* Описание страницы */}
            <div className="mt-7.5 font-semibold shadow-xs p-5 lg:p-7.5 rounded-lg mb-7.5">
...
export default LandscapeCategoryPage;
